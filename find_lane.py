...
def find_lane_skip_window(binary_warped, left_fit, right_fit):
    ...
    y_eval = np.max(np.concatenate((lefty, righty)))
    ...
    # plt.ylim(720, 0)
    # plt.show()
    return result, left_curverad, right_curverad, left_fitx, right_fitx
...
def project_back(orig, warped, Minv, left_fitx, right_fitx):
    if Minv is None or left_fitx is None or right_fitx is None:
        return None
